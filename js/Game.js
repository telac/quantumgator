...
Quantumgator.Game.prototype = {
  ...
  create: function() {
    ...
    this.game.T = 0;
    this.game.temperatureText = this.game.add.text(100, 20, "temperature", {font:"20px Arial", fill:"#000000"});
    this.game.temperatureText.anchor.set(0.5);

    this.game.lanes = this.game.add.group();
    for (i = 0; i < 5; i++) {
      this.game.lanes.create(0, 80+100*i, 'lane');
    }

    this.game.lanes.forEach(function(item) {
